...
/**
 * EngineerLayout — top-navigation shell for the Engineer portal.
 *
 * Guards: redirects to /login/engineer if not authenticated or wrong role.
 * Provides: header with logo, project nav link, language toggle, profile, logout.
 */
export default function EngineerLayout() {
  ...
  if (!isAuthenticated || role !== 'ENGINEER') {
    return <Navigate to="/login/engineer" replace />
  }
        ...
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="ms-2 rounded-full bg-blue-900/40 border border-blue-700/40 px-2 py-0.5 text-[10px] font-semibold text-blue-400">
                {t('role.engineer')}
              </span>
            </div>
          </div>
          ...
          <nav className="hidden items-center gap-1 sm:flex">
            <NavLink
              ...
              className={({ isActive }) =>
                `rounded-lg px-3 py-2 text-sm font-medium transition-colors ${isActive ? 'bg-brand-900/50 text-brand-300' : 'text-slate-400 hover:text-white'}`
              }
            >
              {t('nav.projects')}
            </NavLink>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={toggleLang}
              className="rounded-lg border border-surface-border px-2.5 py-1.5 text-xs font-medium text-slate-400 transition-colors hover:border-brand-500/60 hover:text-white"
            >
              {t('lang.switch')}
            </button>
        ...
        {menuOpen && (
          <div className="border-t border-surface-border bg-surface px-4 pb-3 pt-2 sm:hidden">
            <nav className="flex flex-col gap-1">
              <NavLink to="/engineer/projects" onClick={() => setMenuOpen(false)} className="rounded-lg px-3 py-2 text-sm font-medium text-slate-400 hover:text-white">
                {t('nav.projects')}
              </NavLink>
