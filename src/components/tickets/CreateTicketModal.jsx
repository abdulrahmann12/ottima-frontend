...
import { createTicket, TICKET_TYPES } from '@/api/internalTicketApi'
import { getAllAdmins, getAllEngineers } from '@/api/usersApi'
...
export default function CreateTicketModal({
  ...
  // Reset form & fetch eligible receivers when modal opens
  useEffect(() => {
    if (!isOpen) return
    setTitle('')
    setDescription('')
    ...
    setSaving(false)
    setUploading(false)
    setTicketType(initialType || 'INSTRUCTION')

    const fetchReceivers = async () => {
      setLoadingReceivers(true)
      try {
        const [admSettled, engSettled] = await Promise.allSettled([
          getAllAdmins(0, 100),
          getAllEngineers(0, 100),
        ])

        const adms = (admSettled.status === 'fulfilled' ? admSettled.value.data?.data?.content : []) ?? []
        const engs = (engSettled.status === 'fulfilled' ? engSettled.value.data?.data?.content : []) ?? []

        let list = []
        if (isAdmin) {
          // Admins send to Engineers or other Admins (Engineers first)
          list = [
            ...engs.map((u) => ({ ...u, roleLabel: 'Engineer' })),
            ...adms.map((u) => ({ ...u, roleLabel: 'Admin' })),
          ]
          if (project?.engineerId) {
            setReceiverId(String(project.engineerId))
          } else if (list.length > 0) {
            setReceiverId(String(list[0].userId))
          }
        } else {
          // Engineers send to Admins or other Engineers (Admins first)
          list = [
            ...adms.map((u) => ({ ...u, roleLabel: 'Admin' })),
            ...engs.map((u) => ({ ...u, roleLabel: 'Engineer' })),
          ]
        ...
        setReceivers(list)
        if (list.length === 0 && admSettled.status === 'rejected') {
          setError(apiError(admSettled.reason, 'Failed to fetch receivers list.'))
        }
      } catch (err) {
        setError(apiError(err, 'Failed to fetch receivers list.'))
      } finally {
    ...
    fetchReceivers()
  }, [isOpen, isAdmin, project, initialType])
  ...
  const validate = () => {
    const errs = {}
    if (!receiverId) errs.receiverId = t('validation.required', { defaultValue: 'Receiver is required' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(apiError(err, 'Failed to create internal request.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Modal
      ...
      title={t('tickets.create_title', { defaultValue: 'New Internal Request' })}
      size="lg"
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
        <Alert message={error} variant="error" onClose={() => setError(null)} />

        {/* Receiver + Ticket Type */}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="form-label">
              {t('tickets.receiver', { defaultValue: 'Send To (Receiver)' })} <span className="text-red-500">*</span>
            </label>
            {loadingReceivers ? (
              <div className="input-base text-xs text-gray-400 flex items-center gap-2">
                <Spinner className="w-3.5 h-3.5" /> Loading users...
              </div>
            ) : (
              <SearchableSelect
                id="ticket-receiver"
                value={receiverId}
                onChange={(val) => {
                  const v = typeof val === 'object' && val !== null && 'target' in val ? val.target.value : val
                  setReceiverId(v)
                  setFieldErrors((prev) => ({ ...prev, receiverId: undefined }))
                }}
                error={fieldErrors.receiverId}
                placeholder="— Select receiver —"
                options={receivers.map((u) => ({
                  value: String(u.userId),
                  label: `${i18n.language === 'ar' ? (u.fullNameAr || u.fullNameEn) : (u.fullNameEn || u.fullNameAr)} (${u.roleLabel})`,
        ...
        <div>
          ...
          <input
            ...
            placeholder={t('tickets.title_placeholder', { defaultValue: 'e.g. Urgent site materials expense approval' })}
            value={title}
            onChange={(e) => {
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={t('tickets.description_placeholder', { defaultValue: 'Provide relevant notes, measurements, or invoice breakdown…' })}
            value={description}
        ...
        {/* Cloudinary Dropzone Attachments */}
        <div className="space-y-2.5">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1.5">
              ...
              <p className="text-[11px] text-gray-500">Supports JPG, PNG, WEBP, and PDF documents</p>
            </div>
          </label>
        ...
        <div className="flex flex-wrap justify-end gap-2.5 border-t border-gray-200 pt-4">
          ...
          <Button
            ...
            disabled={uploading}
          >
            {t('tickets.submit_request', { defaultValue: 'Send Request' })}
          </Button>
        </div>
      </form>
    </Modal>
